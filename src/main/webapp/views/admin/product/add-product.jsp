...
<form class="form-grid" method="post" action="${pageContext.request.contextPath}/admin/product/add" enctype="multipart/form-data">
    <div class="form-group"><label for="name">Tên sản phẩm</label><input id="name" class="form-control-eco" name="name" value="${enteredName}" maxlength="255" required></div>
    <div class="form-group"><label for="price">Giá</label><input id="price" class="form-control-eco" type="number" min="0" max="9999999999999999.99" step="0.01" name="price" value="${enteredPrice}" required></div>
    <div class="form-group"><label for="categoryId">Danh mục</label><select id="categoryId" class="form-control-eco" name="categoryId" required><option value="">-- Chọn danh mục --</option><c:forEach items="${categories}" var="cate"><option value="${cate.id}" ${enteredCategory == cate.id ? 'selected' : ''}>${cate.name}</option></c:forEach></select></div>
    <div class="form-group"><label for="image">Ảnh sản phẩm</label><input id="image" class="form-control-eco" type="file" name="image" accept=".jpg,.jpeg,.png,.gif,.webp" required></div>
    <div class="form-group form-group--full"><label for="description">Mô tả</label><textarea id="description" class="form-control-eco" rows="5" name="description" maxlength="255">${enteredDescription}</textarea></div>
